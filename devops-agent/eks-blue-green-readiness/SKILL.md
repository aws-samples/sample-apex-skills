...
4. **Distinguish absence from unconfirmed.** A CRD read (`elbv2.k8s.aws` TargetGroupBinding, external-dns, Karpenter) that returns `403`, or an unreadable IAM trust policy / ELB / Route 53 / ACM fact, is `unconfirmed` (with the reason + the fix), **never** "GREEN / no IRSA / no LB / stateless". **Any** `unconfirmed` gate input is treated as **not-GREEN** — there is no "materiality" carve-out that lets a gate drop an unconfirmed input and report GREEN/AMBER (an unread hard standup blocker such as the EC2 vCPU/instance quota is not an "informational" caveat).
