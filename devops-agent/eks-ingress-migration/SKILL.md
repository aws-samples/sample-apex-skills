...
description: EKS ingress migration assessment — discovers NGINX/Ingress estate, scores migration difficulty 0-100 with re-architecture gate, and generates per-cluster markdown reports for Gateway API, ALB Controller, or ATX migration paths.
  Analyzes ingress controllers, versions, EOL/CVE status, IngressClass inventory, routing
  topology, DNS/certificate integration, and traffic patterns. Triggers on ingress migration
  assessment, NGINX retirement planning, ingress audit, migration difficulty scoring, Gateway
  API readiness, ALB ingress migration, or ATX transformation evaluation.
---
