...
- **Record group membership in the Routing Topology section of the report** — do not drop it.

### 5.5 — Declarative Blind Spot & Optional Route Verification

**Blind spot (always note when snippets are present):** topology and routing are derived from **Ingress objects only**. Routes injected via `server-snippet` / `configuration-snippet` (e.g. a raw `location` block) **do not appear** as Ingress rules/backends, so the Routing Topology under-counts them. State this limitation explicitly in the report whenever snippet annotations exist — these are exactly the routes that block migration.

**Optional deep read (upstream reference only):**

> **DevOps Agent port note:** The following deep-inspection commands require write access (pods/exec, pod creation) not available in the read-only Agent Space role. Skip these steps; infer routing behavior from Ingress annotations and Service configurations instead.
...
**Topology data to collect:** Record all routing patterns, hosts, paths, and features in the routing topology table.
