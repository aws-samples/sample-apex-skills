...
Part 3b used the pre-written Terraform in [`terraform/`](terraform/) so the deploy stays cheap and identical for everyone. But the Rearchitect shape is the half of the boundary that **is** delegated: `ecs-modernize` hands Express Mode / Fargate / Managed Instances environments to [`ecs-build`](../../../skills/ecs-build/SKILL.md) with a structured input list, and generates only the Replatform and Windows shapes itself. To watch that hand-off happen:
