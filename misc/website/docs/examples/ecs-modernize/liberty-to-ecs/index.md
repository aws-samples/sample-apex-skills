...
When it finishes, compare against [`app-after/`](https://github.com/aws-samples/sample-apex-skills/blob/main/examples/ecs-modernize/liberty-to-ecs/app-after), which is the **reference implementation** of the same transformation. Differences are expected and instructive: `app-after` uses JAX-RS resources where a minimal transformation keeps the existing servlets, since converting a working servlet to JAX-RS is a modernization choice rather than a required remediation; the reference also validates input strictly, rejecting non-blank, non-conforming ids with HTTP 400 where the minimal transformation instead substitutes a generated id. What should match is the *blockers being gone*.

### 3b — Deploy the rearchitected app

This is the one part of the exercise with pre-written Terraform, and the asymmetry is the point: the Rearchitect shape (Fargate, `awsvpc`, no stickiness) **is** `ecs-build`'s territory, so a verified implementation lives in [`terraform/`](https://github.com/aws-samples/sample-apex-skills/blob/main/examples/ecs-modernize/liberty-to-ecs/terraform) to keep the deploy cheap and reproducible. Part 2's shape is not, which is why the skill generated it there. [Part 3c](#3c--optional-the-same-app-on-ecs-express-mode) runs the real `ecs-build` hand-off if you want to see it.
...
- [ECS Build Skill](../../../skills/ecs/ecs-build/) — the Rearchitect hand-off target
  - [Service and Deployment](../../../skills/ecs/ecs-build/references/service-and-deployment) — the Express service generation path and its constraints
