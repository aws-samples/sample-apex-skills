...
- **PCI-DSS** — 1-year audit-log retention minimum; **CMK on EBS/S3/EFS holding cardholder data + no PAN in logs (Req 3)** and **mTLS for CHD crossing untrusted networks (Req 4)**; default-deny NetworkPolicy + Security Groups for Pods to segment cardholder-data namespaces (Req 1); ECR Enhanced Scanning (Req 6 + 11) + quarterly ASV external scan + annual penetration test + **segmentation testing (Req 11.4.5; every 6 months for service providers, Req 11.4.6)**; Security Hub PCI-DSS pack; PCI AOC from Artifact.
