...
4. **Admission-webhook exposure (required for the control-plane CVE band below):** confirm whether the validating admission webhook is actually present and reachable — do NOT assume from the version alone. Run `kubectl get validatingwebhookconfigurations` and look for the ingress-nginx entry (default name `ingress-nginx-admission`); cross-check the controller Deployment args and the `ingress-nginx-controller-admission` Service. The webhook ships **enabled by default** in Helm installs. Decide exposure as follows and **record the exact state** (`webhook: exposed | not-exposed | unverified`) in the finding — do not collapse it to a bare present/absent flag:
   - **Exposed** — the `ingress-nginx-admission` `ValidatingWebhookConfiguration` exists **and** the controller is serving it: the `ingress-nginx-controller-admission` Service is present and backed by endpoints, **and** the controller Deployment is started with a `--validating-webhook=<address>` arg (e.g. `--validating-webhook=:8443`, the Helm default). On a `< v1.11.5 / < v1.12.1` controller this makes the CVE-2025-1974 control-plane path live **regardless of route count** (🔴 5 band).
   - **Not exposed (absent or disabled)** — **any** of the following closes the control-plane path: the `ingress-nginx-admission` VWC is **absent entirely** (webhook never installed — e.g. Helm `controller.admissionWebhooks.enabled=false`, which also removes the admission Service and the `--validating-webhook` arg); **or** the `ingress-nginx-controller-admission` Service is absent / has no endpoints; **or** the controller has **no** `--validating-webhook=<address>` arg. Note that `--validating-webhook` takes a **service address string, not a boolean** — the webhook server simply is not started when the value is empty/absent (there is no `--validating-webhook=false` form). In these cases, note it and fall back to the data-plane assessment.
