...
- 🔴 5 (High): **Multiple IngressClasses marked default.** Per the official Kubernetes *Ingress* documentation ([Default IngressClass](https://kubernetes.io/docs/concepts/services-networking/ingress/#default-ingress-class)): *"If you have more than one IngressClass marked as the default for your cluster, the admission controller prevents creating new Ingress objects that don't have an `ingressClassName` specified."* So while more than one default exists, a **classless Ingress fails admission** — it cannot be created until you ensure at most one default IngressClass. *(Version caveat: the `DefaultIngressClass` admission-plugin code was later changed to pick the **newest** default by `creationTimestamp` rather than reject — see [kubernetes/kubernetes#110974](https://github.com/kubernetes/kubernetes/pull/110974) — so the exact behavior can vary by cluster version; the documented, source-of-truth behavior above is rejection.)* Either way the risk is real — **blocked Ingress creation**, or **operator surprise / cutover ambiguity** if a version silently picks a class nobody intended. Remediation is the same: keep exactly **one** default IngressClass.
...
4. **Admission-webhook exposure (required for the control-plane CVE band below):** confirm whether the validating admission webhook is actually present and reachable — do NOT assume from the version alone. `kubectl get validatingwebhookconfigurations` and look for the ingress-nginx entry (default name `ingress-nginx-admission`); cross-check the controller Deployment args for `--validating-webhook` / the `ingress-nginx-controller-admission` Service. The webhook ships **enabled by default** in Helm installs. If it is present on a `< v1.11.5 / < v1.12.1` controller, the CVE-2025-1974 control-plane path is live **regardless of route count** (see the 🔴 5 band). Record webhook present/absent in the finding.
