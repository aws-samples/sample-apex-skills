---
title: "Pre-Migration Scanning Workflow"
description: ""
custom_edit_url: https://github.com/aws-samples/sample-apex-skills/blob/main/skills/graviton-migration/references/scanner-workflow.md
format: md
---

:::info[Source]
This page is generated from [skills/graviton-migration/references/scanner-workflow.md](https://github.com/aws-samples/sample-apex-skills/blob/main/skills/graviton-migration/references/scanner-workflow.md). Edit the source, not this page.
:::
...
The MCP's source scanner is **migrate-ease** ([github.com/migrate-ease/migrate-ease](https://github.com/migrate-ease/migrate-ease), Apache-2.0). It is good, but it has verified blind spots, and it happily returns `"total_issue_count": 0` on a repo it never had enough to scan. The failure mode this workflow exists to prevent: a bare manifest repo (a lone `pom.xml`, no built code) scans "0 issues," and the agent reports **CLEAN**. That verdict is worthless — the scanner saw no compiled artifacts and no dependency contents. Treat a low finding count as a question, never an answer.
...
Call the source scanner against the mounted workspace. It statically inspects the source tree. **Confirm the tool's real registered name from the MCP's `tools/list` — do not assume `migrate_ease_scan`.** Key parameters include `scanner` (required, ∈ `cpp|python|go|js|java` — the five language wrappers the arm/mcp server installs; the underlying migrate-ease repo also has `rust`/`docker` scanners, but the MCP wrapper does **not** expose them and rejects those values with `Unsupported scanner`), `arch`, `git_repo`, `output_format`, and `extra_args`. Note that the MCP tool schema types `scanner` and `arch` as free-form strings with no JSON-Schema enum, so their accepted *values* are validated at runtime (when the call reaches the wrapper), not by the schema — a bad scanner value like `rust` or `foo` fails at scan time with `Unsupported scanner`. The contract still rejects unknown *parameters*, though (`additionalProperties` is false), so a bad keyword like `path=` fails earlier, at call validation. **There is no `path` argument** (as of Aug 2026 — confirm the live schema via the MCP's `tools/list`, since the accepted parameters can change between image releases) — the scan target is the container's `/workspace` mount you set with `-v "$(pwd)":/workspace:ro`; passing `path=` is rejected as an unexpected keyword. Use **`arch=armv8-a`** (the default): `aarch64` is **rejected** — the underlying migrate-ease `--march` accepts a fixed set (as of migrate-ease `e92c735`, Aug 2026: `armv8-a`, `armv8.6-a+sve2`, `armv9-a`, `armv9.2-a`), and any value outside that set exits non-zero. `armv8-a` is the safe baseline for a Graviton readiness scan; confirm the current set against the tool if a run rejects your value. A valid call is `{"scanner":"cpp","arch":"armv8-a"}` (no path arg). It returns a JSON payload with `returncode`, `parsed_results.issue_summary`, and a `workspace_listing`.
...
- **Verified blind spots — do NOT trust a clean scan to cover these:**
...
Parse the dependency manifests directly — this is where the `<classifier>` and native-package blind spots are caught, because layer 1 does not read them. This layer is owned by [dependency-knowledge.md](dependency-knowledge); follow its per-ecosystem procedure (Maven/Gradle, pip/Poetry, npm, Go modules, Cargo). It **must run** on every workload, even when layer 1 returns zero.
...
Hand the verdict table to the cutover runbook. Only workloads with a re-scanned-CLEAN verdict **on the built tree** proceed to the [Karpenter arm64 cutover](karpenter-migration) (a first-pass CLEAN on an un-built or under-scanned tree does NOT qualify); the multi-arch image each needs is produced by the [multi-arch CI pipeline](multi-arch-pipelines).
