---
title: "Karpenter arm64 Node Cutover Runbook"
description: ""
custom_edit_url: https://github.com/aws-samples/sample-apex-skills/blob/main/skills/graviton-migration/references/karpenter-migration.md
format: md
---

:::info[Source]
This page is generated from [skills/graviton-migration/references/karpenter-migration.md](https://github.com/aws-samples/sample-apex-skills/blob/main/skills/graviton-migration/references/karpenter-migration.md). Edit the source, not this page.
:::
...
- If `default` selects its AMI by **`alias`** (e.g. `amiSelectorTerms: [{ alias: al2023@latest }]`, or `bottlerocket@latest`, `al2@latest`, etc. — any AMI-family alias; the alias set expands per Karpenter release, so check the current EC2NodeClass docs), the alias is **architecture-aware**: Karpenter resolves the arm64 variant automatically for arm64 instance types (a single alias resolves *both* the x86_64 and arm64 AMIs, and the arm64 instance gets the arm64 one). No change needed.
...
The canary's image must be a manifest list containing a `linux/arm64` entry — the image produced by the [multi-arch CI pipeline](multi-arch-pipelines). Reference the image by its normal tag; the container runtime on the arm64 node automatically pulls the arm64 variant from the manifest list. Do not hardcode an `-arm64` tag suffix into the manifest — the whole point of a manifest list is one tag, right arch per node. Confirm the tag is multi-arch (the "Check every container base image for an arm64 variant" section of the [scanner workflow](scanner-workflow)) before you roll it out.
...
If any check fails, roll the canary back, return to the [scanner workflow](scanner-workflow) to find the missed blocker, and re-scan.
...
**Voluntary consolidation is a second, self-inflicted source of the same signal corruption — and `on-demand`-only does not fix it.** The Step-1 NodePool sets `consolidationPolicy: WhenEmptyOrUnderutilized`, so a lone canary on a right-sized on-demand node is a prime "underutilized" target that Karpenter can drain and replace mid-soak, injecting latency and availability blips into the exact signals you are judging arch on. Consolidation is graceful (it drains and honors PDBs), so it is milder than an involuntary Spot reclaim, but it is still churn from your own NodePool config. During the canary/soak window, protect the canary from it: annotate the canary pods `karpenter.sh/do-not-disrupt: "true"`, or set the arm64 NodePool to `consolidationPolicy: WhenEmpty` for the validation window, then restore normal consolidation once past the soak (Step 7).
