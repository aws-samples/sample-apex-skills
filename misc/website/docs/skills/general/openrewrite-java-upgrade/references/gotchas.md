---
title: "Settled fixes (do not re-derive)"
description: ""
custom_edit_url: https://github.com/aws-samples/sample-apex-skills/blob/main/skills/openrewrite-java-upgrade/references/gotchas.md
format: md
---

:::info[Source]
This page is generated from [skills/openrewrite-java-upgrade/references/gotchas.md](https://github.com/aws-samples/sample-apex-skills/blob/main/skills/openrewrite-java-upgrade/references/gotchas.md). Edit the source, not this page.
:::
...
**What happens.** JDK 21 removed the internal certificate generator that Netty's `io.netty.handler.ssl.util.SelfSignedCertificate` used, so `new SelfSignedCertificate()` throws at startup: `OpenJdkSelfSignedCertGenerator not supported`. The fix is to put BouncyCastle on the path, which Netty will use instead.
...
- snappy-java and zstd-jni resolve glibc-only symbols on aarch64 (`__strftime_l` for snappy, `__fprintf_chk` for zstd). They `dlopen` successfully and then crash on a cold path, so a smoke test can pass while a real workload later dies.
...
**Do not add `libxcrypt-compat`.** At netty-tcnative **2.0.72.Final** BoringSSL is statically linked, so the library has no `libcrypt.so.1` dependency and `libxcrypt-compat` is a no-op. This overturns an earlier claim that it was "needed on AL2023" at 2.0.69; do not carry that forward at 2.0.72+.
...
| skip libxcrypt-compat | tcnative 2.0.72.Final+ | dynamically linked / older tcnative |
