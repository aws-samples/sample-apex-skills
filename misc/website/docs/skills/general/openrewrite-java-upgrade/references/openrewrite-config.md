---
title: "OpenRewrite plugin configuration"
description: ""
custom_edit_url: https://github.com/aws-samples/sample-apex-skills/blob/main/skills/openrewrite-java-upgrade/references/openrewrite-config.md
format: md
---

:::info[Source]
This page is generated from [skills/openrewrite-java-upgrade/references/openrewrite-config.md](https://github.com/aws-samples/sample-apex-skills/blob/main/skills/openrewrite-java-upgrade/references/openrewrite-config.md). Edit the source, not this page.
:::
...
Run **all three** recipes together. This is a correction to a tempting shortcut: `UpgradeSpringBoot_3_3` alone does **not** reach Java 21. Spring Boot 3's Java floor is 17, so the Boot recipe transitively includes `UpgradeToJava17` and leaves the project at Java 17. To actually land Java 21 you must add `org.openrewrite.java.migrate.UpgradeToJava21` explicitly. `JakartaEE10` does the `javax` to `jakarta` namespace migration; list it too (this is exactly why kit poms list three active recipes). `rewrite-migrate-java` supplies `UpgradeToJava21` and `JakartaEE10`; `rewrite-spring` supplies `UpgradeSpringBoot_3_3`. On a servlet app that imports `javax.*` the Jakarta step does real rewrites; on a reactive app that never touched `javax.*` it is a source no-op, which is expected, not a failure.
