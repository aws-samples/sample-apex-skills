---
title: "ECS Best-Practices Corpus (Shared Knowledge)"
description: ""
custom_edit_url: https://github.com/aws-samples/sample-apex-skills/blob/main/skills/ecs-architect/references/best-practices-corpus.md
format: md
---

:::info[Source]
This page is generated from [skills/ecs-architect/references/best-practices-corpus.md](https://github.com/aws-samples/sample-apex-skills/blob/main/skills/ecs-architect/references/best-practices-corpus.md). Edit the source, not this page.
:::
...
The further left, the less you operate. This table drives the ops-overhead criterion in [model-selection-framework.md](model-selection-framework). Managed Instances patches on a **14-day** cadence, schedulable via EC2 event windows. ([Managed Instances launch post](https://aws.amazon.com/about-aws/whats-new/2025/09/amazon-ecs-managed-instances/))
...
- **Set memory limits deliberately.** A hard `memory` limit too close to real usage causes OOM kills; use `memoryReservation` as the soft floor for placement. See [architecture-design.md](architecture-design#task-sizing-ec2--managed-instances).
...
- **One resource profile per ASG + capacity provider.** Managed scaling bin-packs against the *smallest* instance type in a mixed ASG; larger tasks hang in `PROVISIONING`. Full detail: [capacity-and-scaling.md](capacity-and-scaling#the-mixed-instance-type-asg-constraint).
...
- **Tune min/max healthy percent** to your capacity headroom — 100/200 for zero-loss deploys if you can afford the spare capacity. See [architecture-design.md](architecture-design#service-parameters).
...
- **Service Connect** adds automatic connection draining for east-west traffic during deployments. See [networking-and-eni-density.md](networking-and-eni-density#service-connect-vs-service-discovery).
