...
- **Session persistence.** tWAS server-side session persistence (database persistence / memory-to-memory replication) is cell configuration; database session persistence CAN be reproduced via the properties-file configuration, which softens the parent path's sticky-session constraint when it exists today. Whether it exists is usually not evidenced in the source tree — carry the user-confirmation note from blocker detection through to this presentation.
