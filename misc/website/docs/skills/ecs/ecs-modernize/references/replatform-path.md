---
title: "Module: Replatform Path"
description: ""
custom_edit_url: https://github.com/aws-samples/sample-apex-skills/blob/main/skills/ecs-modernize/references/replatform-path.md
format: md
---

:::info[Source]
This page is generated from [skills/ecs-modernize/references/replatform-path.md](https://github.com/aws-samples/sample-apex-skills/blob/main/skills/ecs-modernize/references/replatform-path.md). Edit the source, not this page.
:::
...
> **Prerequisites:** Scoring and Recommendation ([scoring-and-recommendation.md](scoring-and-recommendation)) — the Replatform path is presented from the `scoring`, `recommendation`, `tech_stack`, and `blockers` blocks; it is never assembled without them
...
- **Session persistence.** tWAS server-side session persistence (database persistence; on 8.5.5 also memory-to-memory replication, which 9.0 removed in favor of eXtreme Scale) is cell configuration; database session persistence CAN be reproduced via the properties-file configuration, which softens the parent path's sticky-session constraint when it exists today. Whether it exists is usually not evidenced in the source tree — carry the user-confirmation note from blocker detection through to this presentation.
...
Evaluate the rows **in order**; the first matching row decides. Rows 1 and 2 are gates; rows 3 and 4 partition the remaining cases completely.
...
|---|---|---|---|
| 4.x | Yes (IIS-hosted web app: ASP.NET Web Forms / MVC / WCF under IIS) | `mcr.microsoft.com/dotnet/framework/aspnet:4.8` (pick the `-windowsservercore-ltsc2019` / `-ltsc2022` tag matching the chosen host/platform version) | The ASP.NET image ships IIS with ASP.NET registered — the app deploys into it unchanged. .NET Framework 4.8 is the in-place-update terminal release of the 4.x line: it runs applications targeting any 4.x version, so one image serves the whole family |
...
3. **AWS Transform for .NET as an optional accelerator** — include it in the parallel presentation with its scope and its effect on effort:
   ...
   - **Assessment/execution boundary (design decision 8):** in the Assessment_Phase this is a **recommendation in the report only**. No Transform job is proposed for adoption, scoped, or started here. Actual transformation is executed exclusively by the code transformation module ([code-transformation.md](code-transformation)) after Execution_Gate passage.
...
- The recommendation block from [scoring-and-recommendation.md](scoring-and-recommendation) is never mutated by this submodule — `variant` mode in particular leaves it untouched.
