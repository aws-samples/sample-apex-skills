...
- The workload is an HTTP/HTTPS web application or API fronted by a load balancer — Express Mode serves a **single traffic-serving container** on **exactly one TCP port**, on **Fargate**. On the **default path (no custom task definition)** the primary container is named `Main` by default (renaming is supported but not recommended) and the container port defaults to 80; the stricter naming and port-mapping contract applies to the custom `taskDefinitionArn` path (see the task-role/sidecar note below).
...
- A **task role, environment variables, or secrets** are not grounds for exclusion — they are first-class on the managed (default) path (`taskRoleArn` on the service; `environment` / `secret` on `primaryContainer`), so needing a task role for S3 or a database does not push a workload off this candidate. **Sidecars and full task-level control** are reached by supplying your own task definition via `taskDefinitionArn`, which Express Mode then uses **as-is** — additional containers and sidecars are permitted alongside the primary container, and Express reads the execution and task roles, logging, CPU, memory, and container definitions from that task definition (so it stops managing the CloudWatch Logs group). On this custom path the task definition **must** satisfy a hard contract: a container named **`Main`** with a **single TCP port mapping that includes both a container port and a port name** (`portMappings.name`, the same field Service Connect uses — it is required here, not optional), and **`FARGATE`** compatibility. Supplying `taskDefinitionArn` is **mutually exclusive** with `primaryContainer` / `executionRoleArn` / `taskRoleArn` / `cpu` / `memory` ([CreateExpressGatewayService](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_CreateExpressGatewayService.html); [how Express Mode works](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/express-service-work.html)). All of these are build-time inputs to carry into the `ecs-build` hand-off.
