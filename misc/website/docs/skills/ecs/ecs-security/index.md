...
- **Days 61-90 (workload + network + image):** roll out `readonlyRootFilesystem`/non-root/`privileged:false`/dropped-capabilities (test first — read-only rootfs breaks apps that expect to write); move tasks to private subnets + VPC endpoints + SG-per-service; enable immutable tags + image signing; enable the applicable AWS Config conformance pack (HIPAA/PCI/NIST/FedRAMP) + validate Security Hub against it (Audit Manager existing setups only — maintenance mode; see [compliance-accelerators.md](references/compliance-accelerators)).
...
- **Don't let the default log mode silently drop your audit evidence.** Since June 25, 2025 the ECS default log driver mode is **`non-blocking`** — under backpressure logs are **dropped silently**, fatal when logs are the compliance evidence. Set `mode: blocking` (or the account default) for audit-critical services and weigh the availability trade-off. See [audit-logging.md](references/audit-logging).
- **Don't conflate the `ssm` and `ssmmessages` VPC endpoints.** `ssm` serves Parameter Store retrieval; **ECS Exec needs `ssmmessages`** — `ssm` alone leaves Exec broken on private networks. See [network-isolation.md](references/network-isolation).
...
| [identity-and-access.md](references/identity-and-access) | Layer 2 — task vs execution vs instance vs infrastructure roles; the "unable to assume the role" error; confused-deputy trust (all-clusters wildcard); `iam:PassRole`; credential delivery; **ECS Exec governance**; operator-side/cluster-admin IAM (ABAC, MFA) |
...
| [network-isolation.md](references/network-isolation) | Layer 5 — `awsvpc` mode, SG-per-task/service, private subnets, VPC endpoints (incl. `ssmmessages`/`kms` for Exec, ECS control-plane endpoints on EC2), VPC Flow Logs, WAF/Shield, multi-tenancy isolation, ingress via ALB/NLB, egress control |
...
| [compliance-accelerators.md](references/compliance-accelerators) | Layer 7b — Config conformance packs + rules for ECS, Security Hub ECS controls, Artifact, Audit Manager (maintenance mode) |
