...
- **`ECS_DISABLE_PRIVILEGED=true`** as an agent env var on hosts where privileged containers are never needed, to defense-in-depth against a privileged task (see [task-container-hardening.md](task-container-hardening)).
