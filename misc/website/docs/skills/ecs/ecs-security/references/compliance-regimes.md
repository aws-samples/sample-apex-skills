...
- **HIPAA** — confirm an active BAA first; prefer **Fargate** for per-task isolation of PHI workloads (no shared kernel); CMK on ECR/EBS/EFS/logs holding PHI; Secrets Manager injection (no plaintext); GuardDuty Runtime Monitoring; 6-year documentation retention (§164.316(b)(2)); AWS Config conformance pack "Operational Best Practices for HIPAA Security" + Security Hub (AWS Audit Manager in maintenance mode, closed to new accounts, existing setups continue — see [compliance-accelerators.md](compliance-accelerators)); hand the auditor the **signed BAA + SOC 2 report** (there is no "HIPAA AOC" — AWS: *"There is no HIPAA certification for a cloud service provider"*, [AWS HIPAA compliance](https://aws.amazon.com/compliance/hipaa-compliance/)).
...
- **FedRAMP** — Moderate (commercial) vs High (GovCloud) is the first question; CMK for all data layers; VPC endpoints to keep traffic on the AWS backbone; Fargate FIPS if in GovCloud; AWS Config conformance packs "Operational Best Practices for FedRAMP(Moderate)" (High is split into "... (High Part 1)" / "... (High Part 2)"; there is no bare "FedRAMP" pack) (AWS Audit Manager in maintenance mode, closed to new accounts — see [compliance-accelerators.md](compliance-accelerators)); confirm the authorizing agency.
