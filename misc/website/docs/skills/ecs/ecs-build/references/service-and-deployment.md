...
description: ""
custom_edit_url: https://github.com/aws-samples/sample-apex-skills/blob/main/skills/ecs-build/references/service-and-deployment.md
format: md
---

:::info[Source]
This page is generated from [skills/ecs-build/references/service-and-deployment.md](https://github.com/aws-samples/sample-apex-skills/blob/main/skills/ecs-build/references/service-and-deployment.md). Edit the source, not this page.
:::
...
- Autoscaling caveat: `ALBRequestCountPerTarget` target tracking is not compatible with the blue/green deployment type -- re-check https://docs.aws.amazon.com/AmazonECS/latest/developerguide/target-tracking-create-policy.html before combining (see [autoscaling.md](autoscaling)).

## Load balancing

- Service `load_balancer { target_group_arn, container_name, container_port }`; target group `target_type = "ip"` for awsvpc tasks.
- Target group hygiene (see [baseline-defaults.md](baseline-defaults)): `deregistration_delay` 30-60s, health check path/matcher explicit, `health_check_grace_period_seconds` on the service for slow starters.
...
- **Resource argument surface (`aws_ecs_express_gateway_service`, verified against provider 6.59.0 schema 2026-08-13):** `execution_role_arn` and `infrastructure_role_arn` required; `task_role_arn`, `health_check_path`, `cpu`, `memory`, `network_configuration`, `scaling_target`, `cluster`, `wait_for_steady_state` optional; container input is the `primary_container` block only (`image`, `container_port`, `environment`, `secret`, `command`, `aws_logs_configuration`, `repository_credentials`). **There is no task-definition argument** -- so a task role, env vars and secrets are all reachable from Terraform, but the `taskDefinitionArn` path (sidecars, full task-level control) is not: that needs an out-of-band API call. Re-check the schema before assuming this gap persists. Omitting `network_configuration` is legal -- ECS then spreads across the default VPC subnets and builds the security groups itself. ECS propagates the resource's tags (including provider `default_tags`) onto what it creates, alongside `AmazonECSManaged=true`, so tag-based discovery is the audit handle for the resources missing from state.
