...
Note (verified 2026-07-09): the ECS Best Practices Guide has **no standalone observability chapter** — observability-relevant guidance is scattered (stdout/stderr logging in the container-images chapter, https://docs.aws.amazon.com/AmazonECS/latest/bestpracticesguide/application.html; VPC Flow Logs for per-task traffic with `awsvpc`, https://docs.aws.amazon.com/AmazonECS/latest/bestpracticesguide/security-network.html; LB health-check tuning pages). Don't cite a BPG observability chapter that doesn't exist.
