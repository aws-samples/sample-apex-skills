...
custom_edit_url: https://github.com/aws-samples/sample-apex-skills/blob/main/skills/ecs-observability/SKILL.md
format: md
---

:::info[Source]
This page is generated from [skills/ecs-observability/SKILL.md](https://github.com/aws-samples/sample-apex-skills/blob/main/skills/ecs-observability/SKILL.md). Edit the source, not this page.
:::
...
**For buffer benchmarks, FireLens rules, and high-throughput patterns, see:** [Log Delivery Reference](references/log-delivery)
...
- Prometheus/Grafana route: ADOT sidecar remote-writes to Amazon Managed Service for Prometheus, visualized in Amazon Managed Grafana — **documented for Fargate and EC2 only; EXTERNAL explicitly unsupported, MI not documented as of 2026-07-10** (see the ADOT row of [references/launch-type-matrix.md](references/launch-type-matrix) for the source quote) — verify before advising MI. Alternative keeping data in CloudWatch: the CloudWatch agent's Prometheus scrape support.
...
**For metric tables, instance-level collection, and AMP vs CloudWatch-agent-Prometheus selection, see:** [Metrics Stacks Reference](references/metrics-stacks)
...
**For migration gotchas, IAM, and Application Signals strategy details, see:** [Tracing and Signals Reference](references/tracing-and-signals)
...
|---|---|
| [log-delivery.md](references/log-delivery) | blocking vs non-blocking details, buffer-size benchmarks, awslogs options, FireLens/Fluent Bit configuration, filesystem buffering, high-throughput logging, routing to third-party destinations (Datadog, Splunk, SIEMs) and awslogs-vs-FireLens selection |
| [metrics-stacks.md](references/metrics-stacks) | Container Insights standard vs enhanced metric tables, GPU/DCGM scoping, instance-level metrics, ADOT+AMP+AMG vs CloudWatch-agent Prometheus, metric cost levers |
| [tracing-and-signals.md](references/tracing-and-signals) | X-Ray maintenance-mode details and migration, ADOT collector setup and IAM, Application Signals sidecar vs daemon, language support |
| [launch-type-matrix.md](references/launch-type-matrix) | any "does X work on launch type Y" question — check before every capability claim |
| [native-visibility-and-exec.md](references/native-visibility-and-exec) | EventBridge events, service events, container health checks, ECS Exec constraints and VPC endpoints, monitoring-plan baseline |
