...
- **`eks-ingress-migration`** (assesses/plans migrating off the NGINX ingress controller to Gateway API / ALB / ATX) — negative 22 ("migrate our nginx ingress to ALB and score the effort"). The discriminator: ingress-migration assesses the ingress data path and migration effort; operational-review rates 10 operational areas GREEN/AMBER/RED.
- **`eks-security`** (EKS security & compliance hardening — CIS, HIPAA/PCI/FedRAMP/GDPR, Pod Identity/Access Entries, PSA, GuardDuty, image signing, audit logging) — negative 23 ("Prepare our EKS cluster for a HIPAA com…").
- **`eks-well-architected-review`** (deterministic Well-Architected review of a live EKS cluster — five pillars, fixed jq detections, severity-weighted 0-100 per pillar plus a coverage gate) — negatives 24, 25 ("run a Well-Architected review of this c…").
<!-- SIBLING_MAP_END -->
