...
- **`eks-cost-intelligence`** (live cost-efficiency assessment with a scored, dollar-quantified report) — negative 9 ("Analyze my cluster's cost efficiency and produce a scored report…").
- **`eks-platform-engineering`** (building an Internal Developer Platform / self-service on EKS) — negative 10 ("Wire up a self-service portal so develo…").
- **`graviton-migration`** (sets up its OWN Arm migration MCP server for arm64 scanning — distinct from the EKS MCP server) — negatives 11, 12 ("Set up the arm migration mcp (arm-mcp)…").
<!-- SIBLING_MAP_END -->
