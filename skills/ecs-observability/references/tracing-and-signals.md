...
> Facts verified 2026-07-09 against https://docs.aws.amazon.com/xray/latest/devguide/xray-daemon-ecs.html and https://docs.aws.amazon.com/xray/latest/devguide/xray-sdk-daemon-timeline.html
...
- **Scope the claim precisely:** only the SDKs and daemon are in maintenance mode. The **X-Ray backend service is not** — traces exported via ADOT/OTel still land in and are analyzed by X-Ray.
...
- **Launch-type scope:** the ECS ADOT/AMP integration page states support for "Amazon ECS workloads hosted on Fargate and Amazon ECS workloads hosted on Amazon EC2 instances. External instances aren't supported currently." (per https://docs.aws.amazon.com/AmazonECS/latest/developerguide/application-metrics-prometheus.html). Managed Instances is not enumerated — verify before advising, and treat EXTERNAL as unsupported.
...
| **Daemon** (one agent service per cluster) | EC2 only — **does not support Fargate** | With `awsvpc`/`bridge` app networking you must wire instance private IPs into app env vars | https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Application-Signals-ECS-Daemon.html |
...
| Managed Instances | ECS "Managed Daemons" exist as a scheduling construct, but observability-collector daemon support is not documented — verify before advising; sidecar patterns are the safer recommendation |
