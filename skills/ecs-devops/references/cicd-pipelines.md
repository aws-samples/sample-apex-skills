...
| AWS-native pipeline, existing CodeDeploy blue/green | Keep CodePipeline "ECS (Blue/Green)" action; plan migration to native (see [controllers-and-migration.md](controllers-and-migration.md)) |
...
- Pair the service with circuit breaker + alarm rollback so a bad image fails the pipeline stage rather than silently degrading (see [failure-detection-and-rollback.md](failure-detection-and-rollback.md)); note the alarm bake time keeps the deployment `IN_PROGRESS` longer — set stage timeouts accordingly.
