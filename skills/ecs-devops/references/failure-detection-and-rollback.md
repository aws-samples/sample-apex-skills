...
4. **Manual re-deploy of the previous task-definition revision** via `UpdateService` — the universal fallback, and the only "rollback" available under the `EXTERNAL` controller (where you instead shift your own task sets — see [controllers-and-migration.md](controllers-and-migration.md)). CodeDeploy-controller rollback reroutes traffic back to the original task set. ([CodeDeploy ECS steps](https://docs.aws.amazon.com/codedeploy/latest/userguide/deployment-steps-ecs.html))
