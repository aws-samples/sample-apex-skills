...
- Choosing a launch/deployment model with no security driver (→ `ecs-architect`), auditing live operational posture (→ `ecs-operation-review`), or discovery/inventory of an estate (→ `ecs-recon`)
...
| **6 — Runtime Security** | Detect at runtime | **GuardDuty ECS Runtime Monitoring** (Fargate + EC2; **not** Managed Instances) + **Extended Threat Detection** (auto, no cost); findings → Security Hub | [runtime-security.md](references/runtime-security.md) |
...
| [identity-and-access.md](references/identity-and-access.md) | Layer 2 — task vs execution vs instance vs infrastructure roles; the "unable to assume the role" error; confused-deputy trust; `iam:PassRole`; credential delivery |
...
| [network-isolation.md](references/network-isolation.md) | Layer 5 — `awsvpc` mode, SG-per-task/service, private subnets, VPC endpoints, ingress via ALB/NLB, egress control |
