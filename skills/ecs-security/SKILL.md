...
- **Preventive governance** — ECS itself won't refuse a non-compliant task definition at launch, so enforce what IAM *can* see with **SCPs / IAM condition keys**: `ecs:RegisterTaskDefinition` supports the `ecs:privileged`, `ecs:compute-compatibility`, `ecs:task-cpu`/`ecs:task-memory`, and tag condition keys — so you can deny `privileged` task definitions ([Service Authorization Reference — ECS](https://docs.aws.amazon.com/service-authorization/latest/reference/list_amazonelasticcontainerservice.html)). **No condition key inspects `environment`/secret contents**, so plaintext-secret registration is *not* IAM/SCP-deniable — catch it with detective/pipeline controls instead (Security Hub ECS.8, AWS Config, cfn-guard/OPA in CI) and require image signing in the pipeline. Add **VPC Flow Logs** on task subnets (assumed by IR; PCI Req 10) and **AWS WAF + Shield** on any internet-facing ALB
