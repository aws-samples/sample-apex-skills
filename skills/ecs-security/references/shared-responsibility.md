...
- **IMDS lockdown — critical, because there is no task isolation on EC2.** Prevent tasks from reaching the EC2 Instance Metadata Service (which would expose the instance role's credentials). Enforce **IMDSv2** (token-required, hop limit 1) on the instance, and follow the ECS [roles recommendations](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/security-iam-roles.html#security-iam-roles-recommendations) to block container access to IMDS. On Fargate this concern does not exist.
- **`ECS_DISABLE_PRIVILEGED=true`** as an agent env var on hosts where privileged containers are never needed, to defense-in-depth against a privileged task (see [task-container-hardening.md](task-container-hardening.md)).
