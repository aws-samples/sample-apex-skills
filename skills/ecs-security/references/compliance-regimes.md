...
3. Pull the attestation (SOC 2 / ISO / PCI AOC / FedRAMP package) from **AWS Artifact** for the auditor. For HIPAA there is **no AOC or certification** — the HIPAA artifacts are the **signed BAA** (accepted via AWS Artifact Agreements) plus supporting reports such as SOC 2 ([AWS HIPAA compliance](https://aws.amazon.com/compliance/hipaa-compliance/)).
