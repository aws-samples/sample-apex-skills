...
description: Runs a deterministic AWS Well-Architected Framework review of an Amazon EKS cluster. Collects live data with kubectl and the aws CLI, scores it across five of the Framework's six pillars (Operational Excellence, Security, Reliability, Performance Efficiency, Cost Optimization; not Sustainability, which is not cluster-observable) using fixed jq detections so scores are reproducible, separates measured findings from governance questions, applies a coverage gate so thin or empty clusters cannot score well, and renders a self-contained Cloudscape-styled HTML report. Use when asked to run a Well-Architected review of an EKS cluster, measure how far it complies with the Framework, score or audit it across the five pillars, or get a prioritized plan to raise that score. Not for operational-posture-only audits (eks-operation-review), cost analysis in dollars (eks-cost-intelligence), fact-only inventory (eks-recon), static advice (eks-best-practices), hardening (eks-security), or design documents (eks-design).
metadata:
  author: terjing
  version: "2.1"
  framework: AWS Well-Architected Framework
  service: Amazon EKS
  compatibility: Requires the aws CLI (authenticated), kubectl with access to the target cluster, jq, and python3 (report renderer). Network access to the AWS account and cluster API is required. All collected data stays local and the rendered report makes no network requests.
allowed-tools: Bash(aws eks describe-cluster:*) Bash(aws eks list-nodegroups:*) Bash(aws eks describe-nodegroup:*) Bash(aws eks list-addons:*) Bash(aws eks describe-addon:*) Bash(aws eks list-fargate-profiles:*) Bash(aws eks list-pod-identity-associations:*) Bash(aws eks update-kubeconfig:*) Bash(aws iam list-open-id-connect-providers:*) Bash(aws ec2 describe-security-groups:*) Bash(aws ec2 describe-subnets:*) Bash(aws ec2 describe-nat-gateways:*) Bash(aws ec2 describe-route-tables:*) Bash(aws ec2 describe-vpc-endpoints:*) Bash(aws ec2 describe-instances:*) Bash(aws ec2 describe-volumes:*) Bash(aws ecr describe-repositories:*) Bash(aws cloudtrail describe-trails:*) Bash(kubectl get:*) Bash(kubectl config view:*) Bash(jq:*) Bash(mkdir:*) Bash(rm:*) Bash(ln:*) Bash(python3:*) Read Write
---
...
[references/workflow.md](references/workflow.md) — it writes `cluster.json`, `pods.json`, `nodes.json`,
...
- **Operational Excellence** — [references/operational-excellence.md](references/operational-excellence.md)
- **Security** (all 54 questions, one consolidated block) — [references/security/identity-access.md](references/security/identity-access.md). The other four security files (data-protection, network, workload-security, governance-compliance) hold per-question rationale and remediation you load when writing findings.
...
- **Cost Opportunities** — from [references/cost-analysis.md](references/cost-analysis.md), in
...
- **Severity weight** (WAF risk): each question is High (3), Medium (2, default), or Low (1) — see `sev()`
  in the reducer, with the full per-question rationale in [references/severity.md](references/severity.md).
