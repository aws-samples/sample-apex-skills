...
- [Porting Knowledge — WebSphere Traditional (tWAS) → Liberty](#porting-knowledge--websphere-traditional-twas--liberty)
...
| `com.ibm.websphere.security.*` helpers, custom TAI | Jakarta Security / Liberty security configuration; custom Trust Association Interceptors need redesign — decision point |
...
- **Step 5 — namespace.** Liberty runs both Java EE 8 (`javax`, features like `javaee-8.0`) and Jakarta EE (`jakarta`, features like `jakartaee-10.0`) generations. Migrating tWAS → Liberty does **not** by itself force the `javax` → `jakarta` sweep — landing on EE 8 features first is a legitimate, smaller increment. When the approved plan targets a Jakarta feature level, the [javax → jakarta section](#javax--jakarta-namespace-migration) applies and lands together with the feature-level choice.
- **Step 6 — verification.** Wire `liberty-maven-plugin` (or the Gradle plugin) into the working branch so each increment is verifiable locally (`liberty:dev` for interactive runs, `liberty:test-start` + failsafe for CI-style checks). Liberty runs anywhere the JDK runs, so local verifiability is ordinary toolchain presence — no tWAS installation is needed to verify the *target*; the tWAS *baseline*, like the .NET Framework baseline, is typically not locally verifiable and follows the [honest-reporting rule](#when-local-verification-is-not-possible--honest-reporting).
...
- Migration Toolkit for Application Binaries (binary scanner): https://www.ibm.com/support/pages/migration-toolkit-application-binaries
...
- Open Liberty server configuration (`server.xml`, features, variables): https://openliberty.io/docs/latest/reference/config/server-configuration-overview.html
- Liberty application bindings and extensions (Liberty supports the traditional in-application `ibm-web-bnd.xml` family; `server.xml` values take precedence when both are present): https://www.ibm.com/docs/en/was-liberty/base?topic=manually-application-bindings
...
- Migrating applications that use concurrency and asynchronous programming models to Liberty (CommonJ WorkManager / asynchronous beans → EE Concurrency): https://www.ibm.com/docs/en/was-liberty/base?topic=mal-migrating-applications-that-use-concurrency-asynchronous-programming-models-liberty
- Liberty Maven plugin (dev mode, test lifecycle): https://github.com/OpenLiberty/ci.maven
