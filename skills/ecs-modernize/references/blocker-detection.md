...
| `process_model` | Depends | `replatform_ok` **only** when the entire process tree is reproduced by **app-bundled, in-image** supervision or scheduling — a supervisor process or scheduler that ships inside the image. `must_fix` when supervision or scheduling is **host-registered** (systemd units, `/etc/cron.d` or host crontab entries, or any host-level init/daemon management), or when correctness depends on host reboot semantics or cooperating OS services that cannot be co-located in one container |
...
- WebSphere traditional distributed sessions — database persistence is server-side configuration: https://www.ibm.com/docs/en/was/9.0.5?topic=sessions-distributed
- WebSphere memory-to-memory (DRS) session replication settings — a Network Deployment clustering feature, also server-side configuration (ND ships in both product lines, so the 8.5.5 and 9.0 lines both document it): https://www.ibm.com/docs/en/was-nd/9.0.5?topic=replication-memory-memory-settings
