...
| Java + WebLogic / WebSphere | Vendor container images (WebLogic / Liberty images) at the matching server version | Vendor descriptors (`weblogic.xml`, `ibm-web-bnd.xml`) bind the app to that server's behavior; a substitute server would demand code/config rework, which this path forbids |
