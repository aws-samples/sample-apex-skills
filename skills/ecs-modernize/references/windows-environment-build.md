...
- **Set the `awslogs` `mode` explicitly.** Since 2025-06-25 an unset mode defaults to `non-blocking`, and a non-blocking driver with no `max-buffer-size` buffers only 1 MiB before it starts discarding lines — which lands precisely on the steady-state verification that follows, where the logs are the diagnosis. Generate `mode` plus, in non-blocking mode, a widened `max-buffer-size`; choose `blocking` only when complete logs matter more than task availability, and say so. This applies identically to the Replatform skeleton ([replatform-environment-build.md](replatform-environment-build.md)).
