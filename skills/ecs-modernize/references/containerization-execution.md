...
|---|---|
| **Base image selection** | The Dockerfile's `FROM` uses the image family and version-parity rule the policy stated (e.g. matching-major `eclipse-temurin` JRE; matching `tomcat` image; `mcr.microsoft.com/dotnet/aspnet` at the matching version; the Windows base-image matrix selection for .NET Framework) |
