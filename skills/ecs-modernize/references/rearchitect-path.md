...
- The workload is an HTTP/HTTPS web application or API fronted by a load balancer — on the **default path (no custom task definition)**, Express Mode's contract is a single traffic-serving primary container (named `Main` by default — renaming is supported but not recommended) with **exactly one TCP port mapping** to the ALB and Fargate compatibility.
