...
>
> **Scope note:** This is *topology* migration for an ECS estate you already understand. For assessing an existing *application* and choosing replatform vs refactor, use `ecs-modernize`. To inventory the estate first, use `ecs-recon`.
...
1. **Inventory first** (use `ecs-recon`): current launch types, ASG/capacity-provider config, network mode, service discovery mechanism.
