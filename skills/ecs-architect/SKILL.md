...
- **Discovering what is already running** (inventory launch types, capacity providers, task defs) — use `ecs-recon`.
...
Do not recommend a model before you have the answers to these. If the workload is an existing estate rather than greenfield, run `ecs-recon` first to gather them, then return here.
