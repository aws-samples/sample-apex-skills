...
- 🔴 5 (High), no workaround (Tier-A): `configuration-snippet`/`server-snippet` (and the other no-equivalent features — Lua, mirror-to-arbitrary-backend, regex rewrite with capture groups, TLS passthrough, mTLS client-cert), plus **Basic Auth → OIDC** (breaks non-interactive clients — see Authentication above). These need **application/code changes** and raise the Re-architecture Gate.
- **Tier-B — score via the rubric, NOT automatically 🔴 5:** **CORS, rate-limit, IP-allowlist** have a faithful workaround (app/backend layer, or WAF / Security Group), so they are rated per the `report-generation.md` §1.3 Feature-Gap rubric — **Impact 2–3 while that workaround can be applied**, escalating to Tier-A (up to 5) only when it cannot (a closed/unmodifiable backend whose loss degrades a live business flow). `report-generation.md` §1.3 is the source of truth for their score.
