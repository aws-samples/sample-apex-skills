...
> **Do not build step math on node scale-up latency.** The "~30s" figure in [autoscaling.md](autoscaling.md) is *instance provisioning* speed, not application-ready speed, and there is no published "maximum absorbable step" for EKS. Treat all three layers as things to **pre-warm ahead of time**, not to rely on reacting in the moment.
...
- **MNG warm pools** (pre-initialized instances) to cut cold-launch latency: [autoscaling.md](autoscaling.md) and the [Autoscaler Selection note in SKILL.md](../SKILL.md#autoscaling-quick-reference).
...
- Circuit breakers and degraded responses on shared dependencies rather than failing readiness cluster-wide (a shared-dependency readiness failure removes *all* endpoints at once — see [reliability-core.md — Probe Anti-Patterns](reliability-core.md#probe-anti-patterns)).
